-- queries --
-- Q1 Fetching all information for a user by username
SELECT * 
FROM tUser
WHERE username = 'manasa';

-- Q2 Getting all posts by a specific user  --
SELECT 
    p.post_id, 
    u.username, 
    p.content, 
...
-- Q3 Fetching all friends of a user with status = 'accepted'
SELECT 
    f.friendship_id,
...
-- Q4 Getting all posts with more than 10 likes

SELECT * from tPosts 
where likes_count>10;

-- Q5 Finding users who have not posted in the last 30 days
SELECT 
    u.user_id,
    u.username,
    u.full_name
...
-- Q6 Calculating average number of posts per user
SELECT 
...
-- Q7 Top 5 users with the most friends
SELECT 
    u.username,
...
-- Q8 Get all comments for a post with user details
SELECT 
    c.comment_id,
    u.username,
    u.full_name,
...
-- Q9 Finding  mutual friends between two users
SELECT 
...
-- Q10 Deleting posts older than 1 year
DELETE FROM tPosts
